...
from investlab.scenarios.rolling_returns_core import run_with_args as run_rolling_returns
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("output/rolling_returns"), help="输出目录"
    )
    parser.add_argument(
...
def run(args: argparse.Namespace) -> int:
    return run_rolling_returns(args)


ROLLING_RETURNS_SCENARIO = ScenarioEntry(
    name="rolling-returns",
    description="Generate rolling annualized return matrices (CSV + HTML)",
    add_arguments=add_arguments,
    run=run,
)

SCENARIO_REGISTRY.register(ROLLING_RETURNS_SCENARIO)
